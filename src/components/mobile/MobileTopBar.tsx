...
export const MobileTopBar: React.FC<MobileTopBarProps> = ({ currentChapter, totalChapters, habitStreak, journeyStarted, onOpenMap }) => {
  const progressPercent = journeyStarted ? Math.round((currentChapter / totalChapters) * 100) : 0;
  return (
    <header className="sticky top-0 z-40 w-full border-b border-amber-500/15 bg-slate-950/90 pt-[env(safe-area-inset-top,6px)] backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-[430px] items-center justify-between px-3 py-2.5">
        <button onClick={onOpenMap} className="flex items-center gap-2 text-right" title="باز کردن نقشه مسیر">
          ...
          <span>
            <span className="flex items-center gap-1.5"><b className="text-[11px] text-amber-200">GOAL DREAM</b><small className="rounded-full bg-slate-800 px-1.5 text-[9px] text-slate-300">{journeyStarted ? `${currentChapter}/${totalChapters}` : `${totalChapters} مرحله`}</small></span>
            <span className="block text-[10px] text-slate-500">{journeyStarted ? 'قدم بعدی، فقط وقتی آماده‌ای' : 'سفر کامل هدف‌گذاری'}</span>
          </span>
        </button>
