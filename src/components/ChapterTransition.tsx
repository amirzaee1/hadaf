    ...
    <motion.aside initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-1 mt-5 overflow-hidden rounded-3xl border text-center" style={{ borderColor: `${theme.accent}40`, background: `${theme.surface}` }}>
      <div className="relative h-28 overflow-hidden border-b border-white/8 bg-[#050811]">
        ...
        <span className="absolute inset-x-0 top-3 text-[10px] font-black" style={{ color: theme.accentSoft }}>
          {isCompleted ? 'درِ مرحلهٔ بعد باز شد' : 'پاسخ را می‌توانی برای بعد بگذاری'}
        </span>
      </div>
      <div className="p-4">
      <span className="mx-auto flex h-10 w-10 items-center justify-center rounded-2xl border" style={{ color: isCompleted ? '#6ee7b7' : theme.accentSoft, borderColor: `${theme.accent}50` }}>{isCompleted ? <CheckCircle2 className="h-5 w-5" /> : <ArrowDown className="h-5 w-5" />}</span>
      <p className="mt-3 text-[13px] leading-7 text-slate-300">{isCompleted ? 'پاسخت ثبت شد. حالا انتخاب ادامه مسیر دست خودت است.' : 'نوشتن پاسخ اختیاری است؛ می‌توانی فعلاً ادامه بدهی و بعداً برگردی.'}</p>
      <button onClick={isLast ? onOpenWorkshop : onNext} className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl text-sm font-black text-slate-950" style={{ background: `linear-gradient(135deg, ${theme.accentSoft}, ${theme.accent})` }}>
        {isLast ? <Sparkles className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}
        <span>{isLast ? 'ورود به کارگاه ساخت هدف' : isCompleted ? 'قدم بعدی را باز کن' : 'ادامه بدون پاسخ'}</span>
      </button>
      </div>
    </motion.aside>
  );
